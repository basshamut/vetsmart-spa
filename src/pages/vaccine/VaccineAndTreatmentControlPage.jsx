...
const VaccineAndTreatmentControlPage = () => {
    ...
    useEffect(() => {
        ...
        const mockRecords = [
            { recordId: 1, patient: { id: 1, name: 'Max' }, type: 'Vacuna', name: 'Rabia', date: new Date('2024-01-15'), veterinarian: 'Dr. Smith', notes: 'Anual' },
            { recordId: 2, patient: { id: 2, name: 'Luna' }, type: 'Tratamiento', name: 'Desparasitación', date: new Date('2024-02-10'), veterinarian: 'Dr. Brown', notes: 'Mensual' },
    ...
    return (
        <div>
            ...
            <div className="vaccine-treatment-control-component">
                ...
                <Button label="Nuevo Registro" icon="pi pi-plus" className="p-button-success" style={{ marginTop: '1rem' }} onClick={handleNewRecord} />

                {/* Dialogo para crear/editar registro */}
                <Dialog visible={displayDialog} style={{ width: '450px' }} header={newRecord ? 'Crear Registro' : 'Editar Registro'} modal onHide={() => handleDialogVisibility(false)} footer={recordDialogFooter}>
                    <div className="p-fluid">
                        <div className="p-field">
                            <label htmlFor="type">Tipo</label>
                            <InputText id="type" value={selectedRecord?.type || ''} onChange={(e) => setSelectedRecord({ ...selectedRecord, type: e.target.value })} />
                        </div>
                        <div className="p-field">
                            <label htmlFor="name">Nombre</label>
                            <InputText id="name" value={selectedRecord?.name || ''} onChange={(e) => setSelectedRecord({ ...selectedRecord, name: e.target.value })} />
                        </div>
                        <div className="p-field">
                            <label htmlFor="date">Fecha</label>
                            <Calendar id="date" value={selectedRecord?.date} onChange={(e) => setSelectedRecord({ ...selectedRecord, date: e.value })} dateFormat="yy-mm-dd" showIcon />
                        </div>
                        <div className="p-field">
                            <label htmlFor="veterinarian">Veterinario</label>
                            <InputText id="veterinarian" value={selectedRecord?.veterinarian || ''} onChange={(e) => setSelectedRecord({ ...selectedRecord, veterinarian: e.target.value })} />
                        </div>
                        <div className="p-field">
                            <label htmlFor="notes">Notas</label>
                            <InputText id="notes" value={selectedRecord?.notes || ''} onChange={(e) => setSelectedRecord({ ...selectedRecord, notes: e.target.value })} />
                        </div>
                    </div>
                </Dialog>
